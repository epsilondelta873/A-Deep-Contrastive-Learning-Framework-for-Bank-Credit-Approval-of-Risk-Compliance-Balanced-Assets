...
def main():
    ...
    1. 解析命令行参数
    2. 加载配置文件
    3. 加载有标签训练数据（仅 train_accepted）
    4. 初始化微调模型并加载预训练编码器
    5. 执行有监督微调
    6. 保存最终模型
    """
    # 解析命令行参数
    parser = argparse.ArgumentParser(description="有监督微调脚本")
    parser.add_argument(
        "--config",
        type=str,
    ...
    print(f"  分类头LR缩放: {config.finetune.head_lr_scale}")
    ...
    valid_loader = dataloaders.get('test')  # 使用测试集作为验证集
    ...
    model_path = os.path.join(output_dir, "finetuned_resnet_model.pkl")
