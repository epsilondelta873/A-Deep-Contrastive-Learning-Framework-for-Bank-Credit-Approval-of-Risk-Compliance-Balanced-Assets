...
> 公开版本说明
> 本仓库仅公开可复现代码、配置文件和目录约定，不包含真实数据、模型权重、TensorBoard 日志以及实验结果汇总文件。相关目录会保留说明文档，方便在本地补齐文件后运行。
...
├── runs/               # TensorBoard 日志，不上传
└── document/           # 论文写作相关材料，非代码复现必需
```
